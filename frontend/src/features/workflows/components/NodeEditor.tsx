                ...
                return (
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <DocPanel
                        docPath="/docs/orchestrator-node.md"
                        title="Orchestrator Node — Tutorial & Docs"
                      />
                    </div>
                    ...
                    <div className="rounded-md border border-indigo-500/20 bg-indigo-500/5 p-3 space-y-3">
                      <p className="text-[10px] font-semibold uppercase tracking-wider text-indigo-500/80">
                        Loop Settings
                      </p>
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="space-y-1.5">
                          <Label className="text-xs">Max Retries / iteration</Label>
                          <Input
                            type="number"
                            min={0}
                            max={10}
                        ...
                        <div className="space-y-1.5">
                          ...
                          <Input
                            ...
                            min={0}
                            className="h-8 text-xs"
                            value={(cfg.retryBackoffMs as number) ?? 1000}
                            onChange={(e) =>
                              handleConfigChange(
                                'retryBackoffMs',
                                Math.max(0, parseInt(e.target.value) || 1000),
                              )
                            }
                          />
                        </div>
                        <div className="space-y-1.5">
                          <Label className="text-xs">Max Tokens (0 = default)</Label>
                          <Input
                            type="number"
                            min={0}
                            className="h-8 text-xs"
                            value={(cfg.maxTokens as number) ?? 0}
                            onChange={(e) =>
                              handleConfigChange('maxTokens', parseInt(e.target.value) || 0)
                            }
                          />
                      ...
                      <div className="space-y-1.5">
                        <Label className="text-xs">
                          ...
                          <span className="text-muted-foreground font-normal ml-1">
                            — JS expr: <code className="font-mono bg-muted px-1 rounded">{'(output, iteration, results, context)'}</code>
                          </span>
                        </Label>
                        <Input
                          className="h-8 text-xs font-mono"
                          placeholder="return output?.done === true;"
                          value={(cfg.terminateWhen as string) ?? ''}
                          onChange={(e) => handleConfigChange('terminateWhen', e.target.value)}
                        />
                        <p className="text-[10px] text-muted-foreground">
                          Leave empty to run all iterations. Agent can also signal{' '}
                          <code className="font-mono bg-muted px-1 rounded">__DONE__</code> to stop early.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="rounded-md border border-violet-500/20 bg-violet-500/5 p-3 space-y-3">
                      <p className="text-[10px] font-semibold uppercase tracking-wider text-violet-500/80">
                        Sub-Agent Spawning
                      </p>
                      <div className="space-y-1.5">
                        <Label className="text-xs">Strategy</Label>
                        <Select
                          value={(cfg.subAgentStrategy as string) ?? 'auto'}
                        ...
                        >
                          ...
                          <SelectContent>
                            <SelectItem value="auto">
                              auto — agent signals{' '}
                              <code className="font-mono">__SPAWN_SUBAGENTS__:[...]</code>
                            </SelectItem>
                            <SelectItem value="disabled">disabled — no auto-spawning</SelectItem>
                          </SelectContent>
                        </Select>
                      </div>

                      {/* continue on sub-agent failure */}
                      <div className="flex items-center gap-2 text-[11px] text-muted-foreground">
                        <input
                          type="checkbox"
                          id="continueOnSubAgentFailure"
                          checked={(cfg.continueOnSubAgentFailure as boolean) ?? false}
                          onChange={(e) =>
                            handleConfigChange('continueOnSubAgentFailure', e.target.checked)
                          }
                          className="accent-primary"
                        />
                        <label htmlFor="continueOnSubAgentFailure">
                          Continue if a sub-agent fails (partial failure tolerance)
                        </label>
                      </div>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="space-y-1.5">
                          {((cfg.subAgents as SubAgentConfig[]) ?? []).map((sa, idx) => (
                            <div
                              key={idx}
                              className="rounded-md border border-border/50 p-2 space-y-1.5 bg-muted/20"
                            >
                              <div className="flex items-center gap-2">
                                {agents.length > 0 ? (
                                  <Select
                                    ...
                                    onValueChange={(v) => {
                                      const next = [
                                        ...((cfg.subAgents as SubAgentConfig[]) ?? []),
                                      ];
                                      next[idx] = { ...sa, agentId: v };
                                      handleConfigChange('subAgents', next);
                                    }}
                                  >
                                    ...
                                    <SelectContent>
                                      {agents
                                        .filter((a) => a.id !== (cfg.agentId as string))
                                        .map((a) => (
                                          <SelectItem key={a.id} value={a.id}>
                                            {a.name}
                                          </SelectItem>
                              ...
                              <Input
                                className="h-7 text-xs"
                                placeholder="Role / specialization hint"
                                value={sa.role ?? ''}
                                onChange={(e) => {
                                  const next = [...((cfg.subAgents as SubAgentConfig[]) ?? [])];
                                  next[idx] = { ...sa, role: e.target.value };
                                  handleConfigChange('subAgents', next);
                                }}
                              />
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="space-y-1.5">
                        {((cfg.toolIds as string[]) ?? []).map((tid, idx) => (
                          <div key={idx} className="flex items-center gap-2">
                            {tools.length > 0 ? (
                              <Select
                                value={tid}
                                onValueChange={(v) => {
                                  const next = [...((cfg.toolIds as string[]) ?? [])];
                                  next[idx] = v;
                                  handleConfigChange('toolIds', next);
                                }}
                              >
                            ...
                            <Button
                              ...
                              className="h-8 w-8 text-destructive shrink-0"
                              onClick={() => {
                                const next = ((cfg.toolIds as string[]) ?? []).filter(
                                  (_, i) => i !== idx,
                                );
                                handleConfigChange('toolIds', next);
                              }}
                    ...
                    {/* Output shape */}
                    <div className="rounded-md bg-muted/30 border border-border/40 p-3 text-[11px] text-muted-foreground font-mono space-y-0.5">
                      <p className="font-semibold text-foreground/70 mb-1 font-sans">Output shape:</p>
                      <p>{'{'}</p>
                      <p className="pl-4">output: any <span className="font-sans">{'// last iteration output'}</span></p>
                      <p className="pl-4">iterations: number <span className="font-sans">{'// how many loops ran'}</span></p>
                      <p className="pl-4">results: Array{'<{'}</p>
                      <p className="pl-8">iteration: number</p>
                      <p className="pl-8">output: any</p>
                      <p className="pl-8">subAgentOutputs?: any[]</p>
                      <p className="pl-4">{'}>'}  </p>
                      <p>{'}'}</p>
                    </div>

                    {/* Signal protocol */}
                    <div className="rounded-md border border-indigo-500/20 bg-indigo-500/5 p-3 space-y-1.5 text-[11px] text-muted-foreground">
                      <p className="font-semibold text-indigo-500 text-xs">Agent Signal Protocol</p>
                      <p>
                        <code className="font-mono bg-muted px-1 rounded text-foreground">__DONE__</code>
                        {' '}— terminate loop immediately
                      </p>
                      <p>
                        <code className="font-mono bg-muted px-1 rounded text-foreground">
                          {'__SPAWN_SUBAGENTS__:[{"agentId":"...","input":{...}}]'}
                        </code>
                        {' '}— spawn sub-agents in parallel (auto strategy only)
                      </p>
                      <p>
                        Each iteration receives{' '}
                        <code className="font-mono bg-muted px-1 rounded text-foreground">_iteration</code>,{' '}
                        <code className="font-mono bg-muted px-1 rounded text-foreground">_previousOutput</code>, and{' '}
                        <code className="font-mono bg-muted px-1 rounded text-foreground">_results</code>{' '}
                        in its input.
                      </p>
                    </div>

                    {/* Inline tutorial tips */}
                    <div className="pt-2 border-t border-border/40">
                      <Collapsible>
                        <CollapsibleTrigger asChild>
                          <Button
                            variant="ghost"
                            className="w-full justify-between items-center text-xs h-8 px-2 font-medium bg-indigo-500/5 hover:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-500/20"
                          >
                            <span className="flex items-center gap-2">
                              <ChevronRight className="h-3.5 w-3.5" />
                              Quick-Start Tips
                            </span>
                            <span className="text-muted-foreground font-normal text-[10px]">5 steps</span>
                          </Button>
                        </CollapsibleTrigger>
                        <CollapsibleContent className="pt-2 animate-in slide-in-from-top-1 space-y-2">
                          {[
                            {
                              step: '1',
                              title: 'Define "done" first',
                              body: 'Write your terminateWhen expression before the prompt. It forces a precise output contract.',
                              tip: 'return output?.status === "done";',
                            },
                            {
                              step: '2',
                              title: 'Give the agent a JSON contract',
                              body: 'Every response must end with a JSON block containing at least a status field. Put __DONE__ after it.',
                              tip: '{ "status": "done", "result": "..." }',
                            },
                            {
                              step: '3',
                              title: 'Start with maxIterations: 1',
                              body: 'Prove the single-shot works before adding loops. Loops hide prompt bugs.',
                              tip: 'Set maxIterations: 1, maxRetries: 0 first.',
                            },
                            {
                              step: '4',
                              title: 'Add sub-agents one at a time',
                              body: 'Wire one sub-agent, verify its output in logs, then add the next.',
                              tip: '__SPAWN_SUBAGENTS__:[{"agentId":"researcher","input":{...}}]',
                            },
                            {
                              step: '5',
                              title: 'Harden with retries last',
                              body: 'Enable maxRetries and simulate a failure once the happy path works.',
                              tip: 'continueOnSubAgentFailure: true for research; false for code generation.',
                            },
                          ].map(({ step, title, body, tip }) => (
                            <div
                              key={step}
                              className="rounded-md border border-border/40 bg-muted/20 p-2.5 space-y-1"
                            >
                              <div className="flex items-center gap-2">
                                <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-indigo-500/20 text-[9px] font-bold text-indigo-600 dark:text-indigo-400">
                                  {step}
                                </span>
                                <p className="text-[11px] font-semibold text-foreground">{title}</p>
                              </div>
                              <p className="text-[10px] text-muted-foreground pl-6">{body}</p>
                              <code className="block text-[10px] font-mono bg-muted/60 rounded px-2 py-1 text-foreground/80 ml-6 truncate">
                                {tip}
                              </code>
                            </div>
                          ))}
                          <div className="flex justify-end pt-1">
                            <DocPanel
                              ...
                              title="Orchestrator Node — Tutorial & Docs"
                              trigger={
                                <button className="text-[10px] text-indigo-500 hover:text-indigo-600 underline underline-offset-2">
                                  Read full tutorial →
                                </button>
                              }
                            />
                      ...
                      <Collapsible>
                        <CollapsibleTrigger asChild>
                          ...
                          >
                            ...
                            <span className="text-muted-foreground font-normal text-[10px]">4 checks</span>
                          </Button>
                        </CollapsibleTrigger>
                        <CollapsibleContent className="pt-2 animate-in slide-in-from-top-1">
                          <div className="rounded-md border border-border/40 bg-muted/20 p-2.5 space-y-2 text-[10px] text-muted-foreground">
                            {[
                              {
                                icon: '🧪',
                                label: 'Unit',
                                text: 'Test agent prompt alone — send a mock input with _iteration:0, _previousOutput:null.',
                              },
                              {
                                icon: '🔌',
                                label: 'Integration',
                                text: 'Use a stub agent (always returns {"status":"done"}) to verify loop logic without real AI.',
                              },
                              {
                                icon: '💥',
                                label: 'Chaos',
                                text: 'Break one sub-agent ID, confirm retries appear in logs; confirm FAILED after maxRetries.',
                              },
                              {
                                icon: '📸',
                                label: 'Regression',
                                text: 'Snapshot-test output shape — catches silent prompt changes before production.',
                              },
                            ].map(({ icon, label, text }) => (
                            ...
                            <div className="flex justify-end pt-1 border-t border-border/30 mt-1">
                              <DocPanel
                                ...
                                title="Orchestrator Node — Tutorial & Docs"
                                trigger={
                                  <button className="text-[10px] text-primary hover:text-primary/80 underline underline-offset-2">
                                    Full testing guide →
                                  </button>
                                }
                              />
