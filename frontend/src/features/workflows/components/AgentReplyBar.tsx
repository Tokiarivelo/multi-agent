...
const ChoiceChips = memo(
  ({ proposals, selected, multi, danger = false, onToggle }: ChoiceChipsProps) => {
    return (
  ...
  useEffect(() => {
    const handler = async (event: MessageEvent) => {
      ...
      try {
        ...
        onDone();
        toast.success(
          t('workflows.agentReply.githubConnected', 'GitHub connected — workflow resuming…'),
        );
      } catch (err) {
...
export const AgentReplyBar = memo(
  ({
    ...
    const parsedFromText = useMemo(() => (agentText ? parseProposals(agentText) : []), [agentText]);
    ...
    const TypeIcon = isCustomOnly ? Pencil : isMulti ? ListChecks : ToggleLeft;
          ...
          <div className="flex items-center gap-2">
            <CustomTextInput
              ...
              placeholder={
                selected.length > 0
                  ? t(
                      'workflows.agentReply.overridePlaceholder',
