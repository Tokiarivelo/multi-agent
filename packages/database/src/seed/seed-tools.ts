...
const tools = [
  ...
  },
  {
    ...
    parameters: [
      ...
      { name: 'selector', type: 'string', description: 'CSS selector to extract specific elements', required: false },
    ],
    code: null,
  },
  {
  ...
  },
  {
    ...
    isBuiltIn: true,
    parameters: [
      { name: 'path', type: 'string', description: 'Path to the PDF file (absolute or relative to workspace root)', required: true },
      { name: 'cwd', type: 'string', description: 'Working directory to resolve relative paths from', required: false },
    ],
    code: null,
  },
  {
    name: 'workspace_read',
    description: "Read a file from the user's currently open local workspace (browser File System Access API proxy)",
    category: ToolCategory.FILE,
    isBuiltIn: true,
    parameters: [
      { name: 'filePath', type: 'string', description: 'Path to the file relative to the workspace root', required: true },
      { name: 'workspaceId', type: 'string', description: 'Optional ID of the workspace. If missing, use active.', required: false },
    ],
    code: null,
  },
  {
    name: 'workspace_write',
    description: "Write content to a file in the user's currently open local workspace (browser File System Access API proxy)",
    category: ToolCategory.FILE,
    isBuiltIn: true,
    parameters: [
      { name: 'filePath', type: 'string', description: 'Destination path relative to the workspace root', required: true },
  ...
  },
  {
    ...
    isBuiltIn: true,
    parameters: [
      { name: 'token', type: 'string', description: 'GitHub Personal Access Token', required: true },
      { name: 'endpoint', type: 'string', description: 'GitHub API endpoint (e.g. /repos/owner/repo)', required: true },
      { name: 'method', type: 'string', description: 'HTTP Method (GET, POST, etc.)', required: false, default: 'GET' },
      { name: 'body', type: 'object', description: 'JSON payload for POST/PATCH requests', required: false },
    ],
    code: null,
  },
  {
  ...
  },
  {
    ...
    parameters: [
      ...
      { name: 'phoneNumberId', type: 'string', description: 'WhatsApp Phone Number ID', required: true },
  ...
  },
  {
    ...
    isBuiltIn: true,
    parameters: [
      { name: 'command', type: 'string', description: 'Shell command string to execute', required: true },
