...
const tools = [
  ...
  },
  {
    ...
    description: 'Generate a document in various formats (PDF, DOCX, XLSX, MD, CSV, HTML, TXT, JSON) and save it to the workspace. Returns localPath (absolute path on disk) and url (cloud download link). Use localPath as the "path" field in email_send attachments to attach the generated file to an email.',
    category: ToolCategory.FILE,
    isBuiltIn: true,
    parameters: [
  ...
  {
    name: 'download_file',
    description: 'Download a file from a URL and optionally save it to the workspace. Returns localPath, url, filename, contentType, size, and base64 content when no outputPath is set.',
    category: ToolCategory.WEB,
    isBuiltIn: true,
    parameters: [
      { name: 'url', type: 'string', description: 'URL of the file to download', required: true },
      { name: 'outputPath', type: 'string', description: 'Workspace-relative or absolute path to save the file (optional)', required: false },
      { name: 'filename', type: 'string', description: 'Override the filename (default: derived from URL)', required: false },
      { name: 'headers', type: 'object', description: 'Optional HTTP headers (e.g. Authorization)', required: false },
    ],
    code: null,
  },
  ...
  {
    name: 'email_send',
    description: 'Send an email via SMTP with optional file attachments. Supports plain text and HTML bodies. To attach files generated by document_generate, use the localPath value from that tool\'s result as the "path" field in each attachment object.',
    ...
    parameters: [
      ...
      { name: 'attachments', type: 'array', description: 'File attachments. Each item is an object with: filename (string, required), path (string — local file path from document_generate localPath, or a URL), content (string — raw text or base64), contentType (string — MIME type e.g. application/pdf). Use path when attaching files from document_generate.', required: false },
    ],
    code: null,
  },
  {
  ...
  },
  {
    ...
    description: 'List all attachments for a specific email by its UID. Returns partId, filename, contentType, and size for each attachment.',
    category: ToolCategory.MCP,
    isBuiltIn: false,
    mcpConfig: {
      serverUrl: '$EMAIL_MCP_URL/api/mcp',
      toolName: 'gmail_list_attachments',
    },
    parameters: [
      { name: 'uid', type: 'string', description: 'UID of the email (from gmail_fetch_emails result)', required: true },
  ...
  },
  {
    ...
    description: 'Download a specific attachment from an email. Returns base64-encoded file content. Optionally saves to a workspace path via the download_file tool.',
    category: ToolCategory.MCP,
    isBuiltIn: false,
    mcpConfig: {
      serverUrl: '$EMAIL_MCP_URL/api/mcp',
    ...
    parameters: [
      { name: 'uid', type: 'string', description: 'UID of the email (from gmail_fetch_emails result)', required: true },
      { name: 'partId', type: 'string', description: 'Part identifier of the attachment (from gmail_list_attachments result)', required: true },
      { name: 'outputPath', type: 'string', description: 'Optional workspace-relative or absolute path to save the file (e.g. downloads/invoice.pdf)', required: false },
      { name: 'mailbox', type: 'string', description: 'Mailbox name (default: INBOX)', required: false },
      { name: 'imapUser', type: 'string', description: 'Gmail address (uses IMAP_USER env if omitted)', required: false },
      { name: 'imapPass', type: 'string', description: 'Gmail App Password (uses IMAP_PASS env if omitted)', required: false },
      { name: 'imapHost', type: 'string', description: 'IMAP host (default: imap.gmail.com)', required: false },
